...
public class CharacterService {
	...
	// - Reader (입력) / Writer (출력) : 최상위 (스트림) 클래스
	...
	public void fileInput() {
		...
		try {
			...
			while(true) {
				// String BufferedReader.readLine() : 한 줄씩 문자열을 읽는 역할(줄단위) == 개행 문자가 나올 때까지의 문자열을 읽음
				line = br.readLine();
				
				// 더이상 읽어올 내용이 없다면 반복 종료
				if(line == null) break;
	...
	public void fileOutput() {
		// 스트림 참조 변수 선언
		FileWriter fw = null; // 문자 기반의 파일 출력용 스트림
